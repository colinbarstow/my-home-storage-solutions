            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    @foreach($blogs as $blog)
                        <tr>
                            <td><a href="{{ route('admin.blogs.show', $blog->blog->id) }}">{{ $blog->blog->title }}</a></td>
                            <td><img src="{{ \Illuminate\Support\Facades\Storage::url($blog->blog->image) }}" width="50px"></td>
                            <td>{{ \Carbon\Carbon::parse($blog->scheduled_time)->format('d-m-Y H:i:s') }}</td>
                            <td>
                                <span style="overflow: visible; position: relative; width: 125px;">
                                    @if(Auth::user()->hasPermissionTo('edit facebook blog posts'))
                                    <a href="{{ route('admin.facebook-posts.edit', $blog->id) }}" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit facebook blog post">
                                    ...
                                    @endif
                                    @if(Auth::user()->hasPermissionTo('edit facebook blog posts'))
                                    <a href="{{ route('admin.facebook-posts.instant-push', $blog->id) }}" class="btn btn-sm btn-clean btn-icon" title="Push instantly to Facebook">
                                        {!! \App\Classes\Theme\Metronic::getSVG('media/svg/icons/Navigation/Arrow-up.svg') !!}
                                    </a>
                                    @endif
