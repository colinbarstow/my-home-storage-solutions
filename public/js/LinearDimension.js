...
export default class LinearDimension {
    ...
    constructor(domRoot, renderer, camera, scene) {
    ...
    create(p0, p1, extrude) {
        ...
        this.nodeIMG = new THREE.Object3D();
    ...
    update(camera) {
        ...
        let origin = pmax.clone().add(pmin).multiplyScalar(0.5);
        let dir = pmax.clone().sub(pmin);
        dir.normalize();

        let length = pmax.distanceTo(pmin) / 2;
        let hex = 0x0;
        let arrowHelper0 = new THREE.ArrowHelper(dir, origin, length, hex, this.config.headLength, this.config.headWidth);
        this.node.add(arrowHelper0);

        dir.negate();
        let arrowHelper1 = new THREE.ArrowHelper(dir, origin, length, hex, this.config.headLength, this.config.headWidth);
        ...
        if (this.domElement !== undefined) {
            ...
            this.domElement.innerHTML = `${this.config.unitsConverter(pmin.distanceTo(pmax)*1000).toFixed(0)}${this.config.units}`;
        }
    }
